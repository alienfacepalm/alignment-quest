...
import type { AlignmentKey, PersonCard, Placements, QuestBoard } from "./types";
...
function isAlignmentKey(value: string): value is AlignmentKey {
 ...
 */
export async function fetchQuestBoardFromLlm(userTopic: string): Promise<QuestBoard> {
...
- JSON must be valid UTF-8; use double quotes for all keys and strings.`;
  ...
  try {
    parsed = JSON.parse(extractJsonObject(content)) as LlmPayload;
  } catch {
  ...
  const used = new Set<AlignmentKey>();
  const topicHash = hashTopicKey(topic);
  const palette: PersonCard[] = rows.map((p, i) => {
    ...
    const tags = Array.isArray(p.clueTags) ? p.clueTags.map((t) => String(t).trim()).filter(Boolean).slice(0, 6) : [];
